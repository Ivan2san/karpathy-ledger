...
**P52. File good answers back into the wiki.** A comparison you asked for, a connection you discovered, an analysis you ran: these are valuable and should not vanish into chat history. File them back as pages. Your explorations compound, not just your sources. The difference between a knowledge base and a notebook. Flagged tension (2026-08-01): this ledger does not follow P52. `AGENTS.md` (Outputs) makes outputs export-only, so nothing an output reasons to is filed back as a principle, and new principles arrive only through ingest of a Karpathy-direct source. The reason is single-author purity (the maintainer's framing is not Karpathy's claim), not disagreement with P52. Recorded rather than resolved, per P79; see P84. `[WIKI]`
